...
var webMapsChannel = Backbone.Radio.channel('webMaps');

webMapsChannel.reply("addLayers", function(mapId, mapOrder){
    //debugger;
    var leafletMap = leafletMaps[mapOrder];
    var webMapM = webMapsC.get(mapId);
    
    if(!leafletMap || !webMapM){
        return;
    }

    if(!leafletMap.hasLayer(webMapM.get("tileLayer"))){

        // 1. add the tile layer
        leafletMap.addLayer(webMapM.get("tileLayer"));

        // 2. add the grid layer (UTF tiles) + the respective grid control
        leafletMap.addLayer(webMapM.get("gridLayer"));
        leafletMap.addControl(webMapM.get("gridControl"));

        // 3. add the legend (if defined in the project in tilemill)

        var legend = webMapM.get("tileJson").legend;
        if(legend){
            leafletMap.legendControl.addLegend(legend);
        }
        
        webMapM.set("active", true);
    }

});

// inverse of the above handler
webMapsChannel.reply("removeLayers", function(mapId, mapOrder){
    //debugger;
    ...
    if(leafletMap.hasLayer(webMapM.get("tileLayer"))){
        ...
        leafletMap.removeLayer(webMapM.get("tileLayer"));

        // 2. add the grid layer (UTF tiles) + the respective grid control
        leafletMap.removeLayer(webMapM.get("gridLayer"));
        leafletMap.removeControl(webMapM.get("gridControl"));
...
var MenuBodyIV = Mn.ItemView.extend({
    ...
    toggleMap: function(e){
        ...
        var mapOrder = this.model.get("mapOrder")

        if($checkBox.is(":checked")){
            webMapsChannel.request("addLayers", mapId, mapOrder);
        }
        else{
            webMapsChannel.request("removeLayers", mapId, mapOrder);
        }
    },
    onAttach: function(){
        //debugger;
...
var MenuLV = Mn.LayoutView.extend({
    ...
    show: function(){
        ...
        //     collection: new Backbone.Collection(Clima.mapsMenu)
        // }));

        var model = new Backbone.Model({
            mapOrder: this.model.get("mapOrder"),
...
var MapIV = Mn.ItemView.extend({
    ...
    initializeMap: function(){
        ...
        var order = this.model.get("mapOrder");
        window.leafletMaps[order] = this.map = L.mapbox.map(div, undefined, {
            center: [38.75, -9.15],
            zoomControl: false,
            attributionControl: false,
            zoom: 8,
...
// this is the default map view; it will always be shows; the user might eventually show a second map (syncronized) in the other region.
var mapOneIV = new MapIV({
    model: mapOneM
});

Cartografia.mapOneRegion.show(mapOneIV);
...
var loadMap = function(mapId){
    debugger;
        ...
        // 1. add the tile layer
        window.lefletMaps[0].addLayer(webMapM.get("tileLayer"));

        // 2. add the legend (if defined in the project in tilemill)
        var legend = webMapM.get("tileJson").legend;
        if(legend){
            window.lefletMaps[0].legendControl.addLegend(legend);
        }
        
        // 3. add the grid layer (UTF tiles) + the respective grid control
        window.lefletMaps[0].addLayer(webMapM.get("gridLayer"));
        window.lefletMaps[0].addControl(webMapM.get("gridControl"));
