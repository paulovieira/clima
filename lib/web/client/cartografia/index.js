...
var MapIV = Mn.ItemView.extend({
    ...
    initializeMap: function(){
        ...
        window.mapOne = this.map = L.mapbox.map(div, undefined, {
            center: [38.75, -9.15],
            zoomControl: false,
            attributionControl: false,
            zoom: 8,
...
var loadMap = function(mapId){
        ...
        window.mapOne.addLayer(Clima.maps[mapId]["tileLayer"]);
debugger;
        var legend = Clima.maps[mapId]["tileLayer"].getTileJSON().legend;
        if(legend){
            window.mapOne.legendControl.addLegend(legend);
        }
        

        window.mapOne.addLayer(Clima.maps[mapId]["gridLayer"]);
        window.mapOne.addControl(Clima.maps[mapId]["gridControl"]);
