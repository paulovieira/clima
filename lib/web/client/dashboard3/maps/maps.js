...
var LegendsShapesLV = Mn.LayoutView.extend({
	template: "maps/templates/legends-shapes.html",
	events: {
		"change select.js-available-shapes": "updateAvailableAttributes"
	},
	regions: {
		availableAttributesRegion: "#available-attributes-region"
	},
	isNumeric: function(){

	},
	updateAvailableAttributes: function(e){
		var x = 1;
debugger;
		var shapeName = $(e.target).val();
		var selectedShape = shapesC.findWhere({tableName: shapeName})
		
		var numericAttributes = selectedShape.get("shapeColumnsData").filter(function(obj){

			var type = obj["data_type"].toLowerCase();
			return /int/.test(type) || /decimal/.test(type) || /numeric/.test(type) || /real/.test(type) || /double/.test(type);
		})
		var legendsAttributesIV = new LegendsAttributesIV({
			collection: new Backbone.Collection(numericAttributes)
		})
		this.availableAttributesRegion.show(legendsAttributesIV);

	}
});

var LegendsAttributesIV = Mn.ItemView.extend({
	template: "maps/templates/legends-attributes.html",
});


var LegendsNoAvailableShapesIV = Mn.ItemView.extend({
	template: "maps/templates/legends-no-available-shapes.html",
})
var LegendsLV = Mn.LayoutView.extend({
	template: "maps/templates/legends.html",
	events: {
		"change select.js-all-maps": "updateAvailableShapes",
		//"change select.js-available-shapes": "updateAvailableAttributes"
	},
	regions: {
		availableShapesRegion: "#available-shapes-region",
//		availableAttributesRegion: "#available-attributes-region"
	},
	updateAvailableShapes: function(e){
		...
		if(availableShapes.length > 0){
			var legendsShapesLV = new LegendsShapesLV({
				collection: new Backbone.Collection(availableShapes)
			})
			this.availableShapesRegion.show(legendsShapesLV);
		}
		else{
			this.availableShapesRegion.show(new LegendsNoAvailableShapesIV());
		}
...
*/
	...
	showMapsLegends: function(){
		...
		Q.all([mapsC.fetch(), shapesC.fetch()])
			.then(function(){ 
				self.tabContentRegion.show(legendsLV);
			})
			.catch(function(err){
				var msg = err.responseJSON ? err.responseJSON.message : 
											( err.message ? err.message : "unknown error" );
