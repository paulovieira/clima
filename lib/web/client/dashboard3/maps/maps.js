...
var LegendChooseAttributeLV = Mn.LayoutView.extend({
	...
	ui: {
		...
		invertColors: "button#js-invert-colors",
		darkenColors: "button#js-darken-colors"
	},
	...
	events: {
		...
		"click @ui.invertColors":         "invertColors",
		"click @ui.darkenColors":         "darkenColors"
	},
	...
	invertColors: function(){
		...
		this.set("scale", reversedScale);

		// when the "invert colors" button is clicked, we know for sure that 
		// the necessary properties are well defined (min, max and scale)
...
//debugger;
	...
	getAttributeStats: function(){
		...
		var promise = $.ajax( {
			url: "/api/v1/shapes-stats",
			method: "PUT",
			contentType: "application/json",
			data: JSON.stringify({
				schema_name: "geo",
				table_name:  this.get("selectedShapeName"),
				column_name: this.get("selectedAttribute")
			})
		});

		// reset max and min
		this.set("selectedAttributeMin", undefined);
		this.set("selectedAttributeMax", undefined);
		...
		Q(promise)
			.then(function(data){ 
				...
				if(!/int/.test(data.columnType)){
					self.colorsRegion.empty();
					alert("Attribute should be integer");
					return;
				}

				self.set("selectedAttributeMin", parseInt(data.min, 10));
				self.set("selectedAttributeMax", parseInt(data.max, 10));

				var attrs = {
					"min": self.get("selectedAttributeMin"),
					"max" : self.get("selectedAttributeMax"),
					"scale": false
				};

				// enable the other select elements
				self.ui.numberOfClasses.prop("disabled", false);
				self.ui.colorScheme.prop("disabled", false);

				self.showLegendColors(attrs);
			})
			.catch(function(err){
				var msg = err.responseJSON ? err.responseJSON.message : 
											( err.message ? err.message : "unknown error" );
...
//debugger;
		...
		if(max - min > 100){
			this.colorsRegion.empty();
			alert("range of the values in the selected attribute is too big!");
			return;
		}

		var valueClasses = Dashboard.utils.valueClasses(_.range(min, max + 1), selectedClass);
		...
		if(_.contains(sequentialSchemes, selectedColorScheme)){
			correctLight = true;
		}

		// get the actual colors (from chroma.js library)
		var chromaScale = chroma.scale(selectedColorScheme).correctLightness(correctLight).domain([0, selectedClass], selectedClass);

		// finally, create a new array of objects with the data ready to used in the template
		attrs["scale"] = valueClasses.map(function(valueClass, i){
			var obj = { 
				"valueClass": valueClass,
				"colorHex": chromaScale(i).hex(),
			}
			return obj;
		});
...
//debugger;
		...
		var availableAttributes = selectedShape.get("shapeColumnsData").filter(function(obj){
		...
		legendChooseAttributeLV.set("selectedAttributeMax", 0);
