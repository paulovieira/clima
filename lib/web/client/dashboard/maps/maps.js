...
var MapsMenuOrderIV = Mn.ItemView.extend({
	template: "maps/templates/map-menu.html",
	ui: {
		chooseMode: "input[type='radio'][name='menu-order-mode']"
	},
	events: {
		"click button#js-save-map-menu": "saveMapsMenu",
		"change @ui.chooseMode": "changeMode"

	},

	// reset the sortable plugin acoording the mode
	changeMode: function(e){
		...
		// make all dragging arrows invisible
		this.$('i[data-items-type="map-item"]').css("display", "none");
		this.$('i[data-items-type="map-group"]').css("display", "none");
		...
		if(mode==="maps"){
			...
			// add the arrows icons before the map name
			this.$('i[data-items-type="map-item"]').css("display", "");
		}
		else if(mode === "groups"){
			...
			// show the arrows icons before the group title
			this.$('i[data-items-type="map-group"]').css("display", "");
		}
		else{
			throw new Error("MapsMenuOrderIV - unknown mode");
		}
	...
	onAttach: function(){
		...
		this.ui.chooseMode.first().trigger("click");
	},
	...
	saveMapsMenu: function(){
		...
		// get the titles of the map groups (in the current order)
		var groupTitles = this.$('ul[data-items-type="map-group"] > li')
							.map(function(){ 
								return $(this).data("id")    
							});

		// note: the order attribute seems to be redundant, since we have an array
		var collection = $.makeArray(groupTitles).map(function(groupName, i){

			order = (groupName === "unpublished") ? -1 : i;

			var obj = { 
				groupName: groupName, 
				order: order,
				mapElems: (self.mapsMenu.sortable("serialize").get())[i]
			};
			return obj;
		});
...
//debugger;
		...
		var chromaColors = Dashboard.utils.colorBrew[colorScheme][selectedClass];
		if(chromaColors===undefined){
			var keys = _.keys(Dashboard.utils.colorBrew[colorScheme]);
...
*/
	showMapsMenu: function(){
		var mapsMenuOrderIV = new MapsMenuOrderIV({
		...
		Q(mapsMenuC.fetch())
			.then(function(){ 
				self.tabContentRegion.show(mapsMenuOrderIV);
			})
			.catch(function(err){
				var msg = err.responseJSON ? err.responseJSON.message : 
											( err.message ? err.message : "unknown error" );
